...
const NAV = {
  ...
  agent: [
    ...
    { id: "profile", label: "Profile", icon: "fa-user" },
    { id: "add-device", label: "Add device", icon: "fa-circle-plus" },
    { id: "myorganization", label: "My Organization", icon: "fa-building" },
  ],
};
...
async function api(path, options) {
  ...
  if (!res.ok) {
    toast(data.detail || "Request failed");
...
async function load() {
  ...
  const [d, t, extra] = await Promise.all(
    requests.map((p) => p.catch(() => [])),
  );
  devices = d || [];
  tickets = t || [];
...
function renderChrome() {
  ...
  document.getElementById("brand-role").textContent = isOrg()
    ? "Organization admin"
    : "Agent";
  ...
  document.getElementById("devices-title").textContent = isOrg()
    ? "All devices"
    : "My devices";
  document.getElementById("tickets-title").textContent = isOrg()
...
function show(view) {
  ...
  if (view === "myorganization") renderAgentOrganization();
}
...
function renderDashboard() {
  ...
  if (isOrg()) {
    grid.innerHTML = [
      ...
      kpi(
        ...
        stats.partial_devices,
        "fa-circle-half-stroke",
        "amber",
      ),
      kpi(
        "New licenses",
        stats.new_license_devices,
        "fa-certificate",
        "purple",
      ),
      ...
      kpi(
        "Under maintenance",
        stats.devices_under_maintenance,
  ...
  grid.innerHTML = [
    ...
    kpi(
      "Fully operational",
      stats.my_active_devices,
      "fa-circle-check",
      "green",
    ),
    kpi(
      "Under maintenance",
      stats.my_devices_under_maintenance,
      "fa-screwdriver-wrench",
      "amber",
    ),
    kpi(
      "New licenses",
      stats.my_new_license_devices,
      "fa-certificate",
      "purple",
    ),
    ...
    kpi(
      "Resolved tickets",
      stats.my_resolved_tickets,
      "fa-circle-check",
      "green",
    ),
    kpi(
      "Total enrollments",
      stats.my_total_enrollments,
      "fa-chart-line",
      "cyan",
    ),
  ].join("");
  ...
  panels.innerHTML = `
        ...
        <div class="card">
            ...
            ${
              tickets
                .slice(0, 5)
                .map((t) => statRow(`#${t.id} ${t.title}`, badge(t.status)))
        ...
        <div class="card">
            ...
            ${
              devices
                .slice(0, 5)
                .map((d) => statRow(d.device_model, badge(d.operational)))
                .join("") || '<div class="empty">No devices assigned</div>'
            }
        </div>`;
}
...
function renderDevices() {
  ...
  const rows = devices
    ...
    .map((d) => [
      ...
      d.imei1,
      badge(d.operational),
      maintenanceBadge(d.under_maintenance),
      d.status,
      d.location,
      isOrg() ? d.agent_name || "Unassigned" : d.open_issues,
    ]);
  table(
    "devices-table",
    [
...
function renderTicketForm() {
  ...
  select.innerHTML = devices.length
    ? devices
        .map(
          (d) =>
            `<option value="${d.id}">${d.device_model} · ${d.imei1}</option>`,
        )
        .join("")
...
function renderComponents() {
  ...
  document.getElementById("ticket-components").innerHTML = list
    .map(
      (c) =>
        `<label><input type="checkbox" value="${c}"> ${c.replace(/_/g, " ")}</label>`,
    )
    .join("");
}
...
async function submitTicket(event) {
  ...
  const payload = {
    ...
    components: [
      ...document.querySelectorAll("#ticket-components input:checked"),
    ].map((i) => i.value),
  };
...
async function renderAgentOrganization() {
  ...
  const agentOrg = stats.my_organization;
  ...
  if (!agentOrg) {
    ...
    formBox.addEventListener("submit", checkOrg);
  } else {
    orgView.classList.remove("hidden");
    document.getElementById("org-name-display").textContent =
      stats.my_organization || "—";
    document.getElementById("org-reg-display").textContent =
      stats.my_organization_registration || "—";
  }
}
